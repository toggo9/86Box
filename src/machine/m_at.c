...
int
machine_at_b1200d_init(const machine_t *model)
{
    int ret;

    ret = bios_load_interleaved("roms/machines/b1200d/u53.bin",
								"roms/machines/b1200d/u54.bin",
								0x000f8000, 32768, 0);
...
int
machine_at_mbc17plus_init(const machine_t *model)
{
    int ret = 0;
    const char* fn;
...
static const device_config_t mbc17plus_config[] = {
    // clang-format off
    {
        .name = "bios_versions",
        .description = "BIOS Versions",
        .type = CONFIG_BIOS,
        .default_string = "bios_203se",
        .default_int = 0,
        .file_filter = "",
        .spinner = { 0 }, /*W1*/
        .bios = {
            { .name = "ROM BIOS Version 1.04 (No Setup)", .internal_name = "bios_104", .bios_type = BIOS_NORMAL,
              .files_no = 1, .local = 0, .size = 32768, .files = { "roms/machines/mbc17plus/bios_104.bin", "" } },
			{ .name = "ROM BIOS Version 2.03SE (w/ Setup)", .internal_name = "bios_203se", .bios_type = BIOS_NORMAL,
              .files_no = 1, .local = 0, .size = 32768, .files = { "roms/machines/mbc17plus/bios_203se.bin", "" } },  
            { .files_no = 0 }
        },
    },
    { .name = "", .description = "", .type = CONFIG_END }
...
const device_t mbc17plus_device = {
    .name          = "Sanyo MBC17-PLUS",
