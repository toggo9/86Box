...
int
machine_at_emerson4412_init(const machine_t *model)
{
    int ret;

    ret = bios_load_interleaved("roms/machines/emerson4412/EMERSON_4412_EVEN.bin",
								"roms/machines/emerson4412/EMERSON_4412_ODD.bin",
								0x000c0000, 262144, 0);
...
int
machine_at_gc113_386sx_init(const machine_t *model)
{
    int ret;

    ret = bios_load_linear("roms/machines/gc113_386sx/QUADTEL-.BIO",
                           0x000f0000, 65536, 0);
...
int
machine_at_mg386sx_init(const machine_t *model)
{
    int ret;

    ret = bios_load_linear("roms/machines/mg386sx/mg-3862002s_quadtel_bios_101466.bin",
						   0x000f0000, 65536, 0);
...
int
machine_at_sx386_init(const machine_t *model)
{
    int ret;

    ret = bios_load_interleaved("roms/machines/sx386/LOW.bin",
								"roms/machines/sx386/HIGH.bin",
								0x000f0000, 65536, 0);
...
int
machine_at_n8810m30_init(const machine_t *model)
{
    int ret;
...
int
machine_at_powerat_init(const machine_t *model)
{
    int ret;

    ret = bios_load_interleaved("roms/machines/powerat/286-TACT-320548-1.bin",
								"roms/machines/powerat/286-TACT-320548-2.bin",
								0x000e0000, 131072, 0);
...
int
machine_at_pwa286c_init(const machine_t *model)
{
    int ret;

    ret = bios_load_interleaved("roms/machines/pwa286c/1.04B(U72).bin",
								"roms/machines/pwa286c/1.04A(U70).bin",
								0x000f0000, 65536, 0);
...
int
machine_at_4l1024k_init(const machine_t *model)
{
    int ret;

    ret = bios_load_interleaved("roms/machines/4l1024k/286-lo.bin",
								"roms/machines/4l1024k/286-hi.bin",
								0x000f0000, 65536, 0);
...
static const device_config_t mb1212c_config[] = {
    // clang-format off
    {
        ...
        .type = CONFIG_BIOS,
        .default_string = "amibios_v1_1",
        .default_int = 0,
        .file_filter = "",
        .spinner = { 0 }, /*W1*/
        .bios = {
            { .name = "AMIBIOS V1.1", .internal_name = "amibios_v1_1", .bios_type = BIOS_NORMAL,
              .files_no = 1, .local = 0, .size = 65536, .files = { "roms/machines/mb1212c/amibios_v1_1.bin", "" } },
			{ .name = "AMIBIOS V1.2", .internal_name = "amibios_v1_2", .bios_type = BIOS_NORMAL,
              .files_no = 1, .local = 0, .size = 65536, .files = { "roms/machines/mb1212c/amibios_v1_2.bin", "" } },  
            { .name = "C&T BIOS Version 125D (10/26/1990)", .internal_name = "ctbios_oct90", .bios_type = BIOS_NORMAL,
              .files_no = 1, .local = 0, .size = 65536, .files = { "roms/machines/mb1212c/ctbios_oct90.bin", "" } },
			{ .name = "C&T BIOS Version 129D3 (01/18/1991)", .internal_name = "ctbios_jan91", .bios_type = BIOS_NORMAL,
              .files_no = 1, .local = 0, .size = 65536, .files = { "roms/machines/mb1212c/ctbios_jan91.bin", "" } },  
            { .files_no = 0 }
        },
    },
    { .name = "", .description = "", .type = CONFIG_END }
...
const device_t mb1212c_device = {
    .name          = "Biostar MB-1212C",
...
int
machine_at_vsc286e_init(const machine_t *model)
{
    int ret;

    ret = bios_load_linear("roms/machines/vsc286e/PC7286 BIOS (AM27C010@DIP32).BIN",
                           0x000e0000, 131072, 0);
...
int
machine_at_etc386sx_init(const machine_t *model)
{
    int ret;

    ret = bios_load_linear("roms/machines/etc386sx/EISATECH_386SX.BIN",
                           0x000f0000, 65536, 0);
...
int
machine_at_pc50_init(const machine_t *model)
{
    int ret;

    ret = bios_load_linear("roms/machines/pc50/cbm-pc50c-bios-v1.01.03-391236-02-9b00.bin",
                           0x000e0000, 131072, 0);
...
machine_at_p3239_init(const machine_t *model)
{
    ...
    ret = bios_load_linear("roms/machines/p3239/p3239.BIN",
                           0x000e0000, 131072, 0);
...
static const device_config_t mill_magictronics386_config[] = {
    // clang-format off
    {
        ...
        .description = "BIOS Versions",
        .type = CONFIG_BIOS,
        .default_string = "amibios_milltronics",
        .default_int = 0,
        .file_filter = "",
        .spinner = { 0 }, /*W1*/
        .bios = {
            { .name = "AMIBIOS (Milltronics)", .internal_name = "amibios_milltronics", .bios_type = BIOS_NORMAL,
              .files_no = 1, .local = 0, .size = 65536, .files = { "roms/machines/mill_magictronics386/amibios_milltronics.bin", "" } },
            { .name = "AMIBIOS (Magitronics)", .internal_name = "amibios_magitronics", .bios_type = BIOS_NORMAL,
              .files_no = 1, .local = 0, .size = 65536, .files = { "roms/machines/mill_magictronics386/amibios_magitronics.bin", "" } },
            { .files_no = 0 }
        },
    },
    { .name = "", .description = "", .type = CONFIG_END }
...
const device_t mill_magictronics386_device = {
    .name          = "Mill/Magitronics 200-03690-412-6A",
    ...
    .speed_changed = NULL,
    .force_redraw  = NULL,
    .config        = &mill_magictronics386_config[0]
};

int
machine_at_acer1116sx_init(const machine_t *model)
{
    int ret;

    ret = bios_load_linear("roms/machines/acer1116sx/A386SX BIOS U10.bin",
                           0x000f0000, 65536, 0);
...
static const device_config_t ch386scd_config[] = {
    // clang-format off
    {
        ...
        .description = "BIOS Versions",
        .type = CONFIG_BIOS,
        .default_string = "amibios_sep89",
        .default_int = 0,
        .file_filter = "",
        .spinner = { 0 }, /*W1*/
        .bios = {
            { .name = "AMIBIOS Version 091589", .internal_name = "amibios_sep89", .bios_type = BIOS_NORMAL,
              .files_no = 1, .local = 0, .size = 65536, .files = { "roms/machines/ch386scd/amibios_sep89.bin", "" } },
            { .name = "AMIBIOS Version 040990", .internal_name = "amibios_apr90", .bios_type = BIOS_NORMAL,
              .files_no = 1, .local = 0, .size = 65536, .files = { "roms/machines/ch386scd/amibios_apr90.bin", "" } },
            { .files_no = 0 }
        },
    },
    { .name = "", .description = "", .type = CONFIG_END }
...
const device_t ch386scd_device = {
    .name          = "Chicony CH-386S-16C/D ",
    .internal_name = "ch386sd",
    .flags         = 0,
    .local         = 0,
    .init          = NULL,
    .close         = NULL,
...
int
machine_at_mxs386_init(const machine_t *model)
{
    int ret;

    ret = bios_load_interleaved("roms/machines/mxs386/MXS_LOW.bin",
								"roms/machines/mxs386/MXS_HIGH.bin",
								0x000f0000, 65536, 0);
